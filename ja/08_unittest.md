...
　設定値の取得方法はプロジェクトによって様々なやり方が考えられるので、1.4.3 時点で scalikejdbc-test では、接続情報の設定に関するモジュールは提供していません。しかし、将来的に Scala におけるデファクトなやり方が確立するなどの状況になれば、何らかのサポートが入る可能性はあります。
